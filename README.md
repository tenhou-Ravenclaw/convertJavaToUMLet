...
**重要**: このツールは学術的・教育的目的でのみ使用してください。

### 利用規約

- **学術目的限定**: このツールは学習、研究、教育目的でのみ使用されることを想定しています
- **著作権遵守**: 他者の著作権を侵害するコードの解析には使用しないでください
- **不正利用禁止**: 以下の行為は固く禁止されています：
  - 許可なく他人のソースコードを解析・リバースエンジニアリング
  - 企業の機密情報や知的財産の無断解析
  - セキュリティの脆弱性を悪用する目的での使用
  - 商用利用における適切な許可なしでの使用

### 免責

- **責任の制限**: 開発者は本ツールの使用によって生じるいかなる損害についても責任を負いません
- **精度について**: 生成されるUML図の正確性は保証されません
- **利用者の責任**: 本ツールの使用は利用者の自己責任で行ってください

### セキュリティ

- **機密情報の取扱い**: 機密性の高いソースコードの入力は避けてください
- **データ保存**: 入力されたコードはブラウザ内でのみ処理され、外部に送信されません

### 法的事項

本ツールを使用することで、上記の利用規約に同意したものとみなされます。適用される法律や規制を遵守してご利用ください。
